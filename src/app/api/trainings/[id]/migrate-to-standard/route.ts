...
/**
 * POST /api/trainings/[id]/migrate-to-standard
 * Migrate a coaching training to standard PMP training
...
export async function POST(
  ...
  try {
    ...
    // Only mentors, leadership can migrate training
    ...
    const training = await prisma.training.findUnique({
      where: { id },
      include: {
        trainee: { select: { id: true, role: true } },
    ...
    if (training.trainingType !== "ONLINE_COACHING") {
      return NextResponse.json(
        { error: "Only ONLINE_COACHING trainings can be migrated" },
        { status: 400 }
      );
    }

    // Check if training is still active (not completed or cancelled)
    if (training.status !== "ACTIVE") {
      return NextResponse.json(
        { error: "Only active trainings can be migrated" },
        { status: 400 }
      );
    }
    ...
    if (!isLeadership && !isMentor) {
      return NextResponse.json(
        { error: "Only assigned mentors can migrate training" },
        { status: 403 }
      );
    }
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      // If trainee is COMPLETED_TRAINEE (from previous coaching), reset to TRAINEE
      if (training.trainee.role === "COMPLETED_TRAINEE") {
